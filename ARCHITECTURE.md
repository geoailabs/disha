...
- **Visual Isolation & Context:** Each visual must contain only the layers and information relevant to its corresponding request, with only necessary geographic context. Do not carry unrelated layers, markers, or visualizations from one requested section into another. Use `layers_to_show` during `export_map_jpeg`/`export_map_png`/`export_map_pdf` to display only the section's required layers (e.g. constant boundary + target feature/route) without deleting, clearing, or removing previously created layers from the interactive map canvas.
